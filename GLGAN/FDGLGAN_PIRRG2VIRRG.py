    ...
    with torch.no_grad():
        for img, gt, gt_e in tqdm(zip(test_images, test_labels, eroded_labels), total=len(test_ids), leave=False):
            ...
            for i, coords in enumerate(
                ...
                # cv2.imwrite('img.jpg', image_patches)
...
def train(epochs, weights=WEIGHTS, save_epoch=2):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (images, labels, images_t, labels_t) in enumerate(train_loader):
            ...
            image_patches = np.asarray(255 * torch.squeeze(images).cpu(), dtype='uint8').transpose((1, 2, 0))
            cv2.imwrite('imgs.jpg', image_patches[:,:,(2,1,0)])
            ...
            image_patches = np.asarray(255 * torch.squeeze(images_t).cpu(), dtype='uint8').transpose((1, 2, 0))
            cv2.imwrite('imgt.jpg', image_patches[:,:,(2,1,0)])
        ...
        #     start_time = time.time()
        #     MIoU = test(test_ids_V, all=False, stride=32)
        #     end_time = time.time()
        #     print('Test Stide_32 time use: ', end_time - start_time)
        #     start_time = time.time()
        #     if MIoU > MIoU_best:
        #         torch.save(model.state_dict(), './Train_Model/UNetFormer_PIRRG2VIRRG_epoch{}_{}'.format(epoch, MIoU))
        #         MIoU_best = MIoU
    print("Train Done!!")
...
#    io.imsave('./Test_Vision/PIRRG2VIRRG_' + str(acc) + '_tile_{}.png'.format(id_), img)
