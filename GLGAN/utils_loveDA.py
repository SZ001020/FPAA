...
LABELS = ["background", "building", "road", "water", "barren", "forest", "agriculture"] # Label names
# ISPRS color palette
...
class ISPRS_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        if random_idx_s in self.label_cache_s.keys():
            ...
            label_s = np.asarray(io.imread(self.label_files_s[random_idx_s]), dtype='int64') - 1
            if self.cache:
        ...
        if random_idx_t in self.label_cache_t.keys():
            ...
            # Labels are converted from RGB to their numeric values
            label_t = np.asarray(io.imread(self.label_files_t[random_idx_t]), dtype='int64') - 1
            if self.cache:
