...
FOLDER = os.environ.get("DATASET_ROOT", "./ISPRS_dataset/")
...
MODEL = os.environ.get("SAM_RS_MODEL", 'UNetformer')
# MODEL = 'FTUNetformer'
# MODEL = 'ABCNet'
# MODEL = 'CMTFNet'

MODE = os.environ.get("SAM_RS_MODE", 'Train')
# MODE = 'Test'
...
# LOSS = 'SEG+BDY'
# LOSS = 'SEG+OBJ'
LOSS = os.environ.get("SAM_RS_LOSS", 'SEG+BDY+OBJ')
...
if DATASET == 'Vaihingen':
    ...
    Stride_Size = 128
    LOVEDA_ROOT = os.environ.get("LOVEDA_ROOT", os.path.join(FOLDER, "LoveDA"))
    LOVEDA_ROOT = LOVEDA_ROOT.rstrip("/") + "/"
    LABELS = ["background", "building", "road", "water", "barren", "forest", "agriculture"]
    N_CLASSES = len(LABELS) # Number of classes
    WEIGHTS = torch.ones(N_CLASSES) # Weights for class balancing
    MAIN_FOLDER = LOVEDA_ROOT + 'Urban/'
...
# Default test paths follow training paths unless overridden.
DATA_FOLDER_TEST = DATA_FOLDER
LABEL_FOLDER_TEST = LABEL_FOLDER
ERODED_FOLDER_TEST = ERODED_FOLDER

if DATASET == 'Urban':
    # Urban test_ids belong to LoveDA Val split in this codebase.
    default_val_root = LOVEDA_ROOT.replace('/Train/', '/Val/')
    if default_val_root == LOVEDA_ROOT:
        default_val_root = os.path.join(os.path.dirname(LOVEDA_ROOT.rstrip('/')), 'Val') + '/'
    LOVEDA_VAL_ROOT = os.environ.get('LOVEDA_VAL_ROOT', default_val_root).rstrip('/') + '/'
    TEST_MAIN_FOLDER = LOVEDA_VAL_ROOT + 'Urban/'
    DATA_FOLDER_TEST = TEST_MAIN_FOLDER + 'images_png/{}.png'
    LABEL_FOLDER_TEST = TEST_MAIN_FOLDER + 'masks_png/{}.png'
    ERODED_FOLDER_TEST = TEST_MAIN_FOLDER + 'masks_png/{}.png'
